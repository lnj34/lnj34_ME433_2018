...
void i2c_master_setup(void) {
    ANSELBbits.ANSB2 = 0;     //Turn off analog for pin B2
    ANSELBbits.ANSB3 = 0;     //Turn off analog for pin B3
    
    I2C2BRG = 53;                  // I2CBRG = [1/(2*FSCK) - PGD]*PBCLK - 2
                                    // FSCK = 400KHz PGD = 104ns PBCLK = 48MHz??
...
void init_expander(){
    ...
    set_expander(CTRL1_XL, 0x82); //Sample rate 1.66kHz, 2g sensitivity, 100Hz filte 0b10000010
    set_expander(CTRL2_G, 0x88);  //Sample rate 1.66kHz, 1000dps sensitivity 0b10001000
...
void I2C_read_multiple(unsigned char address, unsigned char registerad, unsigned char * data, int length){
    int i = 0;
    i2c_master_start();
    i2c_master_send(address << 1);  //writing
    i2c_master_send(registerad);          //specifying register address
    i2c_master_restart();   
    
    i2c_master_send((address << 1) | 1); //reading from register
    for(i=0; i<length; i++){
